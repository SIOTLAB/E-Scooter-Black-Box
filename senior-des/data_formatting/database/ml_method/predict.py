...
def score_generator(breakdown):
    ...
    return scores
def process_new_data(raw_data_path, filename):
    new_directory = pre_processing(raw_data_path)
    feature_extraction_matrix, _ = feature_extraction_file(new_directory, filename)
...
def predict_new_data(new_data_path, filename, model_file):
    new_data_prepared = process_new_data(new_data_path, filename)
    predictions = load_and_predict(model_file, new_data_prepared)
    return predictions

def predict(filename):
    new_data_path = '../data_raw/test_data/'
    
    model_file_knn = 'knn_model.joblib'
    model_file_dt = 'dt_model.joblib'
    model_file_lr = 'lr_model.joblib'
    model_file_ann = 'ann_model.joblib'
    model_file_svm = 'svm_model.joblib'
    model_file_nb = 'nb_model.joblib'
    model_file_rf = 'rf_model.joblib'
    model_file_gb = 'gb_model.joblib'

    predictions_knn = predict_new_data(new_data_path, filename, model_file_knn)
    predictions_dt = predict_new_data(new_data_path, filename, model_file_dt)
    predictions_ann = predict_new_data(new_data_path, filename, model_file_ann)
    predictions_lr = predict_new_data(new_data_path, filename, model_file_lr)
    predictions_svm = predict_new_data(new_data_path, filename, model_file_svm)
    predictions_nb = predict_new_data(new_data_path, filename, model_file_nb)
    predictions_rf = predict_new_data(new_data_path, filename, model_file_rf)
    prediction_gb = predict_new_data(new_data_path, filename, model_file_gb)
    ...
    breakdown = rough_riding_breakdown(predictions_knn, predictions_dt, predictions_ann, predictions_lr, predictions_svm, predictions_nb, predictions_rf, prediction_gb)
    print(breakdown)

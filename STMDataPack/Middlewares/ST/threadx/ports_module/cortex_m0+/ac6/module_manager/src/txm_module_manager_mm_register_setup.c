...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_region_size_get                 Cortex-M0+      */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*    for the Cortex-M0+ MPU specification.                               */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_calculate_srd_bits              Cortex-M0+      */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Scott Larson, Microsoft Corporation                                 */
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_mm_register_setup               Cortex-M0+      */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Scott Larson, Microsoft Corporation                                 */
...
UINT    i;
    ...
    /* Set address register to user mode entry function address, which is guaranteed to be at least 256-byte aligned.
    ...
    /* Set the attributes, size (256 bytes) and enable bit.  */
    module_instance -> txm_module_instance_mpu_registers[TXM_MODULE_MPU_KERNEL_ENTRY_INDEX].txm_module_mpu_region_attribute_size = TXM_MODULE_MPU_CODE_ACCESS_CONTROL | (_txm_module_manager_region_size_get(256) << 1) | TXM_ENABLE_REGION;
    ...
    /* Determine code block sizes. Minimize the alignment requirement.
       ...
       4. Smallest power of 2 that exceeds the remaining space, minimum 256. */
    ...
    for (i = 0; i < TXM_MODULE_MPU_CODE_ENTRIES; i++)
    {
        ...
        /* Last MPU block is the smallest power of 2 that exceeds the remaining space, minimum 256.  */
        else
        {
...
UINT    i;
    ...
    /* Set base register to user mode entry, which is guaranteed to be at least 256-byte aligned.  */
    base_address_register =  (ULONG) _txm_module_manager_user_mode_entry;
    
    /* Mask address to proper range,  region 0, set Valid bit. */
    base_address_register =  (base_address_register & 0xFFFFFF00) | mpu_register | 0x10;
    module_instance -> txm_module_instance_mpu_registers[0] =  base_address_register;
    
    /* Attributes: read only, write-back, shareable, size 256 bytes, region enabled. */
    module_instance -> txm_module_instance_mpu_registers[1] =  0x0607000F;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_inside_data_check               Cortex-M0+      */
/*                                                           6.1.10       */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Scott Larson, Microsoft Corporation                                 */
